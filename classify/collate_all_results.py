...
test_gpcrs = ground_truth[(ground_truth['gnaoAmp'] == '?') & (ground_truth['Class'] == 'A')]
test_gpcrs = test_gpcrs['HGNC'].tolist()
gprot_in_order = ['gnaoAmp', 'gnaqAmp', 'gna15Amp', 'gnas2Amp', 'gnas13Amp']

# Read all validation results, collate, and get the ROC AUC. 
basedir = 'predictions_binary_amp/'
gprots = os.listdir(basedir)
outdir = 'collated_results_binary_amp/'
...
gprots = os.listdir(basedir)
outdir = 'collated_results_binary_amp/'
outf = open(os.path.join(outdir, 'predictions_binary_amp_regression.csv'), 'w+')
outf_r = open(os.path.join(outdir, 'predictions_binary_amp_rounded.csv'), 'w+')
...
basedir = 'interpretation_binary_amp/'
outdir = 'collated_results_binary_amp/interpretations/'
if not os.path.exists(outdir):
    os.makedirs(outdir)
